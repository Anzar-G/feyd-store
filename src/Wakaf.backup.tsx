...
const Wakaf: React.FC = () => {
  ...
  const buildRequestWaText = () => {
    return [
      "Assalamu'alaikum, saya ingin meminta wakaf Al-Qur'an.",
      `Nama Lembaga: ${orgName}`,
      `Jenis Lembaga: ${orgType}`,
      `Alamat Lengkap: ${orgAddress}`,
      `Jumlah Mushaf yang Diminta: ${requestedCount}`,
      `Nama PIC: ${picName}`,
      `Nomor WA PIC: ${orgWa}`,
      `Email PIC: ${orgEmail}`,
      `Alasan Permintaan: ${reason}`,
      '',
      'Saya akan mengirimkan dokumen pendukung di chat WhatsApp ini.'
    ].join('\n');
  };
  ...
  const validateForm = () => {
    ...
    if (!/^\d+$/.test(whatsapp.replace(/\D/g, ''))) errors.whatsapp = 'Nomor WhatsApp tidak valid';
    if (!email.trim()) errors.email = 'Email wajib diisi';
    if (!/\S+@\S+\.\S+/.test(email)) errors.email = 'Email tidak valid';
    if (!amount) errors.amount = 'Nominal wajib diisi';
  ...
  const validateRequestForm = () => {
    ...
    if (!orgName.trim()) newErrors.orgName = 'Nama lembaga wajib diisi';
    if (!orgType) newErrors.orgType = 'Jenis lembaga wajib dipilih';
    if (!orgAddress.trim()) newErrors.orgAddress = 'Alamat lengkap wajib diisi';
    if (!requestedCount || Number(requestedCount) < 1) newErrors.requestedCount = 'Jumlah mushaf minimal 1';
    if (!picName.trim()) newErrors.picName = 'Nama PIC wajib diisi';
    if (!orgWa.trim()) newErrors.orgWa = 'Nomor WA PIC wajib diisi';
    if (!/^\d+$/.test(orgWa.replace(/\D/g, ''))) newErrors.orgWa = 'Nomor WhatsApp tidak valid';
    if (!orgEmail.trim()) newErrors.orgEmail = 'Email PIC wajib diisi';
    if (!/\S+@\S+\.\S+/.test(orgEmail)) newErrors.orgEmail = 'Email tidak valid';
    if (!reason.trim()) newErrors.reason = 'Alasan permintaan wajib diisi';
    setErrors(newErrors);
  ...
  const handleRequestSubmit = (e: React.FormEvent) => {
    ...
    if (validateRequestForm()) {
      // Open WhatsApp with the request message for both admin numbers
      const waMessage = buildRequestWaText();
      const encodedMessage = encodeURIComponent(waMessage);
      ...
      window.location.hash = '/wakaf/permintaan/terima-kasih';
    }
  };
  
  // Hapus komponen yang tidak digunakan di akhir file
  const MessageCircleIcon = MessageCircle;
  const TruckIcon = Truck;
  
  const handleRequestBlur = (field: string) => {
    ...
    switch (field) {
      case 'orgName':
        if (!orgName.trim()) newError.orgName = 'Nama lembaga wajib diisi';
        break;
      case 'orgType':
        if (!orgType) newError.orgType = 'Jenis lembaga wajib dipilih';
        break;
      case 'orgAddress':
        if (!orgAddress.trim()) newError.orgAddress = 'Alamat lengkap wajib diisi';
        break;
      case 'requestedCount':
        if (!requestedCount || isNaN(Number(requestedCount)) || Number(requestedCount) < 1) 
          newError.requestedCount = 'Jumlah mushaf minimal 1';
        break;
      case 'picName':
        if (!picName.trim()) newError.picName = 'Nama PIC wajib diisi';
        break;
      case 'orgWa':
        if (!orgWa.trim()) {
          newError.orgWa = 'Nomor WA PIC wajib diisi';
        } else if (!/^\d+$/.test(orgWa.replace(/\D/g, ''))) {
          newError.orgWa = 'Nomor WhatsApp tidak valid';
        }
        break;
      case 'orgEmail':
        if (!orgEmail.trim()) {
          newError.orgEmail = 'Email PIC wajib diisi';
        } else if (!/\S+@\S+\.\S+/.test(orgEmail)) {
          newError.orgEmail = 'Email tidak valid';
        }
        break;
      case 'reason':
        if (!reason.trim()) newError.reason = 'Alasan permintaan wajib diisi';
        break;
    }
            ...
            <div className="mt-4">
              ...
              <div className="mt-1 flex items-center">
                ...
                <span className="ml-2 text-sm text-gray-500 truncate">
                  {paymentProofName || 'Belum ada file yang dipilih'}
                </span>
              </div>
          ...
          <div className="max-w-2xl mx-auto px-4">
            <div className="bg-white rounded-2xl border p-5 shadow-sm">
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <label className="text-sm font-medium text-gray-700">Dokumen Pendukung (Opsional)</label>
                  <input type="file" className="mt-1 w-full border rounded-lg px-3 py-2 bg-white" />
                </div>
              </div>
              <div className="mt-4">
                <button
